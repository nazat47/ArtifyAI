...
const ModelsList = ({ models }: ModelsListProps) => {
  ...
  const onHandleDeleteModel = async (
    id: number,
    model_id: string,
    ...
    if (success) {
      toast.success("Model deleted", { id: toastId });
    }
  };

  if (data?.length === 0) {
    return (
      <Card className="flex flex-col h-fit items-center justify-center text-center">
        <CardHeader>
          <CardTitle>No Models Found</CardTitle>
          <CardDescription>
            You have not trained any models yet. Start by creating a new moedel.
          </CardDescription>
            ...
            <CardContent className="flex-1 p-0 pt-3">
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-lg bg-muted px-3py-2">
