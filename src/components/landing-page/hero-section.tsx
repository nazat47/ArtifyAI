...
const MarQueeColumn = ({
  ...
  return (
    <Marquee
      ...
      vertical
      className={cn(
        "w-full relative h-full flex flex-col items-center justify-center",
        className
      )}
      style={{
        //@ts-ignore
    ...
    >
      {Images.sort(() => Math.random() - 0.5).map((image, i) => (
        <Image
          ...
          alt={image.alt}
          priority
          className="w-full h-full object-cover opacity-[0.25] hover:opacity-100 transition-opacity duration-300 ease-in-out"
        />
      ))}
    </Marquee>
  );
...
const HeroSection = () => {
  return (
    <section className="w-full relative overflow-hidden min-h-screen flex items-center justify-center flex-col">
      <div className="relative w-fit px-6 xs:px-8 sm:px-0 mx-auto flex flex-col items-center justify-center space-y-4 text-center z-40 backdrop-blur-[2px]">
        ...
        <p className="mx-auto max-w-3xl text-sm xs:text-xs sm:text-lg md:text-xl mb-8 text-gray-600">
