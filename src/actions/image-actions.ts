...
export async function generateImageAction(
  ...
  if (!process.env.REPLICATE_API_TOKEN) {
    return {
      error: "Replicate token is not set",
      success: false,
      data: null,
    };
  }
  ...
  const modelInput = input.model.startsWith("nazat47/")
    ? {
        model: "dev",
        prompt: input.prompt,
        lora_scale: 1,
...
export async function getImages(limit?: number) {
  ...
  const imageWithUrls = await Promise.all(
    data.map(
      async (img: Database["public"]["Tables"]["generated_images"]["Row"]) => {
        const { data } = await supabase.storage
          .from("artify")
          .createSignedUrl(`${user.id}/${img.image_name}`, 3600);
...
export async function deleteImage(id: string, imageName: string) {
  ...
  await supabase.storage.from("artify").remove([`${user.id}/${imageName}`]);
